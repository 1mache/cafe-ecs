...
void logTriggeredControls(bagel::ent_type ent, const cafe::SdlEvents& input)
{
    using cafe::Controls;

    if (isTriggeredEvent(ent, Controls::Quit))
        std::cout << "[SensorInputTest] Control: Quit\n";

    if (isTriggeredEvent(ent, Controls::KeyDown))
    {
        std::cout << "[SensorInputTest] Control: KeyDown scancode="
                  << SDL_GetScancodeName(input.keyScancode) << "\n";
    }

    if (isTriggeredEvent(ent, Controls::KeyUp))
    {
        std::cout << "[SensorInputTest] Control: KeyUp scancode="
                  << SDL_GetScancodeName(input.keyScancode) << "\n";
    }

    if (isTriggeredEvent(ent, Controls::MouseButtonDown))
    {
    ...
    if (isTriggeredEvent(ent, Controls::MouseButtonUp))
    {
...
int main()
{
    ...
    auto inputEnt = bagel::Entity::create();
    inputEnt.add(SdlEvents{});

    auto sensorEnt = createSensorEntity();
    auto visitorEnt = createVisitorEntity({ -4.f, 0.f });
    ...
    while (isRunning)
    {
        ...
        eventGatheringSystem(renderer);
        const auto& input = inputEnt.get<SdlEvents>();
        logTriggeredControls(inputEnt.entity(), input);

        if (isTriggeredEvent(inputEnt.entity(), Controls::Quit))
            isRunning = false;

        if (isTriggeredEvent(inputEnt.entity(), Controls::KeyDown))
        {
            if (input.keyScancode == SDL_SCANCODE_ESCAPE)
            {
                isRunning = false;
            }
            else if (input.keyScancode == SDL_SCANCODE_E && !sensorEnabled)
            {
                enableSensor(sensorEnt.entity());
                sensorEnabled = true;
            }
            else if (input.keyScancode == SDL_SCANCODE_D && sensorEnabled)
            {
                disableSensor(sensorEnt.entity());
                sensorEnabled = false;
            }
        }

        if (isTriggeredEvent(inputEnt.entity(), Controls::MouseMotion) ||
            isTriggeredEvent(inputEnt.entity(), Controls::MouseButtonDown))
        {
            const WorldPos worldPos =
                screenToWorldPoint(input.mousePos, RenderContext::getCameraPos());
            moveVisitor(visitorEnt, worldPos);
        }

        PhysicsContext::step(dt);
        sensorGatheringSystem();
        syncVisitorTransform(visitorEnt);

        const bool isOverlapping = visitorEnt.has<InSensorArea>();
        const int sensorId = isOverlapping ? visitorEnt.get<InSensorArea>().sensor.id : -1;
        if (isOverlapping != wasOverlapping || sensorId != lastSensorId)
        {
            if (isOverlapping)
            {
        ...
        renderBox(renderer,
                  sensorEnt.get<Transform>(),
                  sensorEnabled ? SDL_Color{ 70, 130, 180, 140 }
                                : SDL_Color{ 60, 60, 60, 140 });
        renderBox(renderer,
                  visitorEnt.get<Transform>(),
