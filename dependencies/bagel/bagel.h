...
{
/**** Parameters ****/
inline constexpr int  MaxComponents     = 20;
...
using mask_type =
    std::conditional_t<MaxComponents <= 8,
                       std::uint_fast8_t,
...
public:
    ...
    int ctz() const
    {
        return _mask ? std::countr_zero(_mask) : -1;
    }
...
class World final : public NoInstance
{
    ...
    static void cleanQuery(int q)
    {
        ...
        {
            if (mask(_queries[q].entities[curr]).test(_queries[q].mask))
            {
                return;
            }

            _queries[q].entities[curr] = _queries[q].entities.pop();
        }
    }
...
public:
    ...
    {
        return _queries[q].curr >= _queries[q].entities.size();
    }
    ...
    static void addComponent(ent_type ent, const T& comp)
    {
        ...
        {
            if ((!prev.test(_queries[q].mask)) && (next.test(_queries[q].mask)))
            {
...
public:
    ...
    void destroy() const
    {
        World::deleteEntity(_ent);
    }

    const Mask& mask() const
    {
        return World::mask(_ent);
    }
    ...
    void addAll(const T& val, const Ts&... vals) const
    {
        add(val);
    ...
    bool has() const
    {
        return mask().test(Component<T>::Bit);
    }
    bool test(const Mask& m) const
    {
        return mask().test(m);
