...
namespace cafe
{
inline constexpr float HEAT_TIME = 5.5f; // default seconds to fully heat a pat
 ...
 *  busy => a pat is inside; timer counts up to heatTime; cooking is the
 *  PastryType captured on intake, re-spawned (heated) on spit-out. */
struct Microwave
{
    bool          busy{ false };
    float         timer{ 0.f };                 // seconds elapsed this heating cycle
    float         heatTime{ HEAT_TIME };        // effective heat time (upgrades lower it)
